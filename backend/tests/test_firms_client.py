...
async def test_process_firms_csv_and_deduplication():
    ...
    # Custom CSV with different confidence values
    ...
    csv_data += "45.0,-120.0,310.5,1.0,1.0,2026-08-27,1200,N,VIIRS,n,2.0,290.0,15.5,D\n"
    csv_data += "45.0,-120.0,310.5,1.0,1.0,2026-08-27,1205,N,VIIRS,h,2.0,290.0,15.5,D\n"
    csv_data += "45.0,-120.0,310.5,1.0,1.0,2026-08-27,1210,N,VIIRS,l,2.0,290.0,15.5,D\n"

    try:
        # Clear out TEST_SOURCE
        async with pool.acquire() as conn:
            await conn.execute("DELETE FROM fire_observations WHERE source = 'FIRMS_TEST_SOURCE'")
            await conn.execute("DELETE FROM hotspots WHERE source = 'FIRMS_TEST_SOURCE'")
        ...
        # Verify confidence normalization and hotspot count
        async with pool.acquire() as conn:
            hotspot = await conn.fetchrow("SELECT observation_count FROM hotspots WHERE source = 'FIRMS_TEST_SOURCE'")
            assert hotspot['observation_count'] == 3

            obs = await conn.fetch("SELECT confidence FROM fire_observations WHERE source = 'FIRMS_TEST_SOURCE' ORDER BY observed_at")
            assert obs[0]['confidence'] == 'nominal'
            assert obs[1]['confidence'] == 'high'
            assert obs[2]['confidence'] == 'low'
        ...
        assert result2["deduplicated"] == 3

        # Verify hotspot count did NOT increment
        async with pool.acquire() as conn:
            hotspot_after = await conn.fetchrow("SELECT observation_count FROM hotspots WHERE source = 'FIRMS_TEST_SOURCE'")
            assert hotspot_after['observation_count'] == 3
...
@patch("app.engine.firms_client.httpx.AsyncClient")
@patch("app.api.v1.ingestion.FIRMS_ENABLED", True)
@patch("app.core.config.FIRMS_SOURCES", ["VIIRS_SNPP_NRT", "VIIRS_NOAA20_NRT", "VIIRS_NOAA21_NRT", "MODIS_NRT"])
def test_firms_ingestion_live_sources(mock_client_class, mock_start_scheduler):
    mock_response = MagicMock()
    mock_response.text = get_sample_csv()
    mock_response.raise_for_status.return_value = None
    ...
    with TestClient(app) as client:
        ...
        assert "VIIRS_SNPP_NRT" in data["metrics"]
        assert "VIIRS_NOAA20_NRT" in data["metrics"]
        assert "VIIRS_NOAA21_NRT" in data["metrics"]
        assert "MODIS_NRT" in data["metrics"]
...
@pytest.mark.anyio
async def test_process_firms_modis_confidence_and_deduplication():
    # Verify MODIS integer confidence logic and deduplication
    pool = await asyncpg.create_pool(os.environ["DATABASE_URL"])
    csv_data = "latitude,longitude,brightness,scan,track,acq_date,acq_time,satellite,instrument,confidence,version,bright_t31,frp,daynight\n"
    csv_data += "40.0,-100.0,310.5,1.0,1.0,2026-08-27,1200,Terra,MODIS,85,6.1,290.0,15.5,D\n" # high
    csv_data += "40.0,-100.0,310.5,1.0,1.0,2026-08-27,1205,Terra,MODIS,50,6.1,290.0,15.5,D\n" # nominal
    csv_data += "40.0,-100.0,310.5,1.0,1.0,2026-08-27,1210,Terra,MODIS,10,6.1,290.0,15.5,D\n" # low

    try:
        async with pool.acquire() as conn:
            await conn.execute("DELETE FROM fire_observations WHERE source = 'FIRMS_MODIS_TEST'")
            await conn.execute("DELETE FROM hotspots WHERE source = 'FIRMS_MODIS_TEST'")

        result = await process_firms_csv(csv_data, pool, "MODIS_TEST")
        assert result["fetched"] == 3
        assert result["accepted"] == 3
        assert result["deduplicated"] == 0

        async with pool.acquire() as conn:
            obs = await conn.fetch("SELECT confidence FROM fire_observations WHERE source = 'FIRMS_MODIS_TEST' ORDER BY observed_at")
            assert obs[0]['confidence'] == 'high'
            assert obs[1]['confidence'] == 'nominal'
            assert obs[2]['confidence'] == 'low'

        # Process second time (should be deduplicated)
        result2 = await process_firms_csv(csv_data, pool, "MODIS_TEST")
        assert result2["fetched"] == 3
        assert result2["accepted"] == 0
        assert result2["deduplicated"] == 3
